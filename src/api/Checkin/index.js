...
const EmployeeCheckinApi = {
  getAllAttendance(employeeId) {
    return trackPromise(axios.get(`/${EMPLOYEE_CHECKIN_NAMESPACE}/${employeeId}`));
  },
  employeeCheckin(body) {
    return trackPromise(axios.post(`/${EMPLOYEE_CHECKIN_NAMESPACE}/in`, body));
  },
  employeeCheckout(id, body) {
    return trackPromise(axios.put(`/${EMPLOYEE_CHECKIN_NAMESPACE}/out/${id}`, body));
  },
  employeeCheckinoutOfADate(body) {
    return trackPromise(axios.post(`/${EMPLOYEE_CHECKIN_NAMESPACE}/in-out`, body));
  }
}
